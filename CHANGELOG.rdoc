...
* New Features
  ...
  * Add Mechanize::Page::Link#rel and #rel?(kind) to read and test the
    rel attribute.
  * Add Mechanize::Page#canonical_uri to read a <link rel="canonical">
    tag.
  * Provide a way to only follow permanent redirects (301)
    automatically: agent.redirect_ok = :permanent   GH #73

  * Add support for Robots Exclusion Protocol (i.e. robots.txt).  New
    methods are: Mechanize#robots_allowed? and #robots_disallowed?.
    Automatic exclusion can be turned on by setting:
      agent.robots = true
